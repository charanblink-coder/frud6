...
def load_custom_css():
    ...
    <style>
        ...
        /* 4. GLASS CONTAINERS (Kept for inner pages, removed from Login) */
        .glass-container {
        ...
        div[data-testid="column"] .stButton button {
            ...
            transition: all 0.2s ease !important; /* Faster transition for click feel */
        }
        ...
        /* CLICK ANIMATION FOR BIG BLOCKS */
        ...
        /* 6. STANDARD BUTTONS & CLICK ANIMATION */
        .stButton button {
        ...
        .stButton button:active {
            transform: scale(0.90) !important; /* Shrink effect */
            box-shadow: 0 2px 5px rgba(37, 99, 235, 0.5) !important;
    ...
    if "splash_shown" not in st.session_state:
        ...
        with splash.container():
            ...
            <div style="height: 90vh; display: flex; flex-direction: column; justify-content: center; align-items: center;">
                ...
                <h1 style="font-size: 4rem; font-weight: 900; color: #fff; letter-spacing: -2px;">FRAUD DETECTION</h1>
                <p style="font-size: 1.5rem; color: #94a3b8; letter-spacing: 5px; margin-top: 10px;">SYSTEM LOADING</p>
            </div>
            """, unsafe_allow_html=True)
            time.sleep(2.5)
        splash.empty()
...
# ------------------------------------
def auth_page():
    # Use columns to center the login area
    c1, c2, c3 = st.columns([1, 1.2, 1])
    
    with c2:
        st.markdown("<br><br>", unsafe_allow_html=True)
        
        # Removed the .glass-container wrapper here as requested
        # Now elements float directly on the background
        
        st.markdown("""
        <div style="text-align: center; margin-bottom: 40px;">
            <div style="font-size: 6rem; margin-bottom: 10px; text-shadow: 0 0 30px rgba(59, 130, 246, 0.5);">💳</div>
            <h2 style="font-weight: 900; font-size: 3rem; letter-spacing: -1px; margin-bottom: 0;">LOGIN</h2>
            <p style="color: #94a3b8; font-size: 1.1rem;">SECURE ACCESS PORTAL</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with tab_login:
            ...
            # No st.form to prevent Enter key submission
        ...
        with tab_signup:
            ...
            register = st.button("Create Account", use_container_width=True)

            if register:
                if not email or not new_user or not new_pass: st.error("Please fill all fields")
                elif new_pass != confirm_pass: st.error("Passwords do not match")
                elif "users" in st.session_state and new_user in st.session_state["users"]: st.error("Username taken")
                else:
...
def selection_page():
    st.markdown(f"""
    <div class="nav-bar">
        <div style="font-size: 1.8rem; font-weight: 900; letter-spacing: -1px;">🛡️ FRAUD DETECTION</div>
        <div style="background: rgba(255,255,255,0.1); padding: 8px 20px; border-radius: 30px; font-weight: bold;">User: {st.session_state.get('current_user', 'Admin')}</div>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("<h3 style='text-align: center; margin-bottom: 60px; opacity: 0.7; letter-spacing: 3px;'>SELECT MODE</h3>", unsafe_allow_html=True)

    col1, col2 = st.columns(2, gap="large")

    with col1:
        if st.button("⚡\n\nINSTANT CHECK", use_container_width=True):
            st.session_state["app_mode"] = "manual"
            safe_rerun()

    with col2:
        if st.button("📂\n\nFILE UPLOAD", use_container_width=True):
...
def instant_check_page(model, model_columns):
    st.markdown('<div class="nav-bar"><div>⚡ INSTANT CHECK</div></div>', unsafe_allow_html=True)
    ...
    with c2:
        ...
        time_val = st.number_input("Time (seconds)", value=0)
    
    st.markdown("<br>", unsafe_allow_html=True)
    if st.button("CHECK FOR FRAUD", use_container_width=True):
        with st.spinner("Analyzing..."): time.sleep(1.2)
        
        errors = []
        if not is_valid_luhn(card_number): errors.append("Invalid Card Number")
        valid_date, msg = validate_expiry(expiry)
        if not valid_date: errors.append(msg)
        ...
        if errors:
            ...
            if pred == 1:
                ...
                st.success(f"✅ TRANSACTION SAFE (Probability: {1-prob:.2%})")
                st.balloons()
...
def batch_upload_page(model, model_columns):
    st.markdown('<div class="nav-bar"><div>📂 FILE UPLOAD</div></div>', unsafe_allow_html=True)
    ...
    if uploaded_file:
        ...
        if st.button("ANALYZE FILE", use_container_width=True):
            st.success("Analysis Complete - No Fraud Found (Mock)")
            st.balloons()
